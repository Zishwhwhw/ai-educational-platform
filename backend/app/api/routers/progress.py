...
from app.services.coins import calculate_coins_earned, can_earn_coins
from app.services.points import apply_fifty_percent_rule, get_level
...
def complete_lesson(
    prog: schemas.ProgressCreate, current_user: CurrentUser, db: Session = Depends(get_db)
):
    user = current_user
    if not user:
